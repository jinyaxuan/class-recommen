...
import content_recall.Tool_func.{CalRuleFunc, GetToday, TimeCover, transform_curriculum}
...
object hit_process {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //返回一个左规则
    val RuleLeft = CalRuleFunc(context,">'2016-09-01'","rule_left",today).filter($"city_id".equalTo(cityId))
    //以广州市为例
    //val RuleLeft = context.sql("select * from tss.rule_test where city_id = '"+ cityId +"'")
    val RuleRight = context.sql("select * from tss.recom_recall where recall != '{}' and city_id = '"+ cityId +"'")
    val Location = context.sql("select sid as ssid,depart_4 from tss.loc_user_department where city_id = '"+ cityId +"'")
    ...
    val recResult: RDD[Row] = RuleLocUnion.map(row => (row.getAs[String]("sid"),row))
      .groupByKey
      .map{
        agroup => {
          val sid = agroup._1
            ...
            if(flag) {
              ...
              for(loc <- locList){
                if(clajsonObj.containsKey(loc)){
                  val claListStr: com.alibaba.fastjson.JSONArray = clajsonObj.getJSONArray(loc)
    ...
    //val readData = context.sql("select * from tss.loc_rule_rec where city_id ='"+ cityId +"'")
    val FlatData: RDD[Row] = filterNullDf.flatMap{
      row => {
    ...
    val allClass = context.sql("select cla_id as class_id," +
      ...
      "where cla_id is not null and cla_id !='' and cla_id != '-' and city_id = '"+ cityId +"' ")
    //拼接cla_current表
    ...
    //课程排期表转换
    val Curriculum = transform_curriculum(context,cityId)
    ...
    val SidRegister = context.sql("Select " +
      ...
      "start_times," +
      "end_times " +
      "from tss.sid_register where reg_isdeleted = '0' and city_id = '"+ cityId +"'")
    ...
    val claScore = context.sql("select * from tss.cla_mjq_hot_score where city_id ='"+ cityId +"'")

    val hitRdd = flatRecDf.join(claScore,flatRecDf("rec_class_id")===claScore("cla_id"),"inner")
      .select("sid",
        "cla_id",
        "city_id",
        "cla_class_type",
        ...
        "cla_classdate_name",
        "fin_score")
      .map(row => (row.getAs[String]("sid"),row))
      .groupByKey
      .map{
        agroup =>{
          var pack_on =  new ArrayBuffer[Map[String,String]]()
          var pack_off =  new ArrayBuffer[Map[String,String]]()
          val sid = agroup._1
          val giter = agroup._2.toIterator
          while(giter.hasNext){
            ...
            info.+=("cla_subject_ids"->arow.getAs[Long]("cla_subject_ids").toString)
            info.+=("cla_class_type"->arow.getAs[Long]("cla_class_type").toString)
            ...
            info.+=("getScore"->arow.getAs[Float]("fin_score").toString)

            if(arow.getAs[Long]("cla_class_type")==1L)
              pack_on.+=(info)
            else
              pack_off.+=(info)
          }
          var pack_on_off = Map[String,ArrayBuffer[Map[String,String]]]()
    ...
    hitDf.foreachPartition{
      partition: Iterator[Row] => {

        val pool_test = getPool.pool
        val jedis_entity = pool_test.getResource
        partition.foreach{
          record => jedis_entity.setex("recommend_studentid_"+record.getAs[String]("sid"),2592000,record.getAs[String]("jsonstring"))
        }
        jedis_entity.close()
      }
    }
